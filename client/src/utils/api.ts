...
export const api = {
  ...
  addGuest: (token: string, name: string, address: string) =>
    request<Guest>('/guests', {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
  ...
  updateGuest: (token: string, id: string, name: string, address: string) =>
  ...
  getGuestBySlug: (slug: string) =>
    request<Guest>(`/guests/slug/${slug}`),

  submitRSVP: (data: RSVPData) =>
    request<{ message: string }>('/rsvp', {
      method: 'POST',
      body: JSON.stringify(data),
    }),

  getRSVPs: () =>
    request<RSVPItem[]>('/rsvp'),

  getConfig: () =>
    request<Config>('/config'),

  updateConfig: (token: string, theme: string) =>
    request<Config>('/config', {
      method: 'PUT',
      headers: { Authorization: `Bearer ${token}` },
  ...
  getEvent: () =>
    request<EventData>('/event'),

  updateEvent: (token: string, data: Partial<EventData>) =>
    request<EventData>('/event', {
      method: 'PUT',
      headers: { Authorization: `Bearer ${token}` },
      body: JSON.stringify(data),
